...
#functions
m=os.getcwd()
if not os.path.exists(m+'/Dicnary sounds'):
    os.mkdir(m+'/Dicnary sounds')
m=m+'/Dicnary sounds'
def shaw():
    if os.name=='nt':
        os.system('cls')
...
#stpath=r'C:\Users\الشركة العربية\Vscod folde\storage'
...
if not os.path.exists(stpath+'/token.txt'):
    if os.path.exists('token.txt'):
        ...
        print('there are dangerous problems\n')    
        main=False 
if not os.path.exists(stpath+'/f.csv'):
    with open(stpath +'f.csv','w') as f:
            pass
if not os.path.exists(stpath+'/artefi.txt'):      
    with open(stpath+'/artefi.txt','w') as ar:
            ar.write('False')            
...
while main:
    ...
    if ch=='1' or ch=='١':
            ...
            ch1p=0                
            while True:
                ein=input('enter(E)xit , (S)pell , (F)avorite words\nenter:')
                if ein.lower()=='e':
                    shaw()
                    break
                if ch1p==3:
                ...
                if ein.lower()=='s':
                                    ...
                                    if internet:
                                        ...
                                        name=0
                                        while True:
                                         if not os.path.exists(str(name)+'.mp3'):
                                             break
                                         name += 1
                                        pi=input('enter word \nor word:translated word(arabic)\nenter:')
                                        if len(pi.split(':'))==2:
                                            spword=pi.split(':')
                                            se=spword[1]
                                            if  se.isascii():
                                                sp1=gTTS(text=spword[1],slow=False)
                                                sp2=gTTS(text=spword[0],slow=False,lang='ar')
                                                sp1.save(f'{name}.mp3')
                                                sp2.save(f'ar{name}.mp3')
                                                shaw()
                                                print(f'done,saved in dicnary sounds(name:{name})')
                                        elif len(pi)>0:
                                            sp1=gTTS(text=pi,slow=False)
                                            sp1.save(f'{name}.mp3')
                                            shaw()
                                            print(f'done,saved in dicnary sounds(name:{name})')
                                        else:
                ...
                #favourite words 1  
                    ...
                    while True:
                        ...
                        print('1-show all my favourite words')
                        print('2-enter new word')
                        l=input('enter your choice:')
                        shaw()
                        if l=='1':
                            with open(stpath+'/f.csv','r') as f:
                                rc=csv.reader(f)
                                for l in rc:
                                    if len(l)>1:
                                        if not artefi:
                            ...
                            cf=input('your favourite word:translated word(arabic)\n')
                            if len(cf.split(':'))==2:
                                op=cf.split(':')
                                with open('f.csv','a',newline='',encoding='utf-8') as f:
                                    wc=csv.writer(f)
                                    wc.writerow(op)
                                    shaw()
                                    print('done')
         ...
         while True:
             ...
             if ink not in wana:
                 ...
                 for i in range(0,10):
                     ...
                     if nu==0:
                         print(f' what is ({eword}) means in arabic\n')
                         ac=False
                         bc=False
                         cc=False
                         ...
                         nuw=i
                         if i =='A':
                                 choi.append(f'B-{laword[0]}' if not artefi else f'{laword[0]}-B')
                                 choi.append(f'C-{laword[1]}' if not artefi else f'{laword[1]}-C')
                                 choi.append(f'D-{laword[2]}' if not artefi else f'{laword[2]}-D')
                                 ac=True
                                 bc=False
                                 cc=False
                                 dc=False
                                 ...
                                 choi.append(f'A-{laword[0]}' if not artefi else f'{laword[0]}-A')
                                 choi.append(f'C-{laword[1]}' if not artefi else f'{laword[1]}-C')
                                 choi.append(f'D-{laword[2]}'if not artefi else f'{laword[2]}-D')
                                 ac=False
                                 bc=True
                                 ...
                                 choi.append(f'A-{laword[0]}' if not artefi else f'{laword[0]}-A')
                                 choi.append(f'B-{laword[1]}' if not artefi else f'{laword[1]}-B')
                                 choi.append(f'D-{laword[2]}' if not artefi else f'{laword[2]}-D')
                                 ac=False
                                 bc=False
                                 cc=True
                                 ...
                                 choi.append(f'A-{laword[0]}' if not artefi else f'{laword[0]}-A')
                                 choi.append(f'B-{laword[1]}' if not artefi else f'{laword[1]}-B')
                                 choi.append(f'C-{laword[2]}' if not artefi else f'{laword[2]}-C')
                                 ac=False
                                 bc=False
                                 cc=False
                                 dc=True
                                 ...
                                 if nuw.lower()==ion.lower():
                                     ...
                                     cori[eword]=aword         
                     if nu==1:
                         print((f' what is ({aword}) means in arabic\n'))
                         ac=False
                         bc=False
                         cc=False
                         i=random.choice(choice)                         
    ...
    #spell situation
        ...
        if internet:
            ...
            while True:
                ...
                if len(pi.split(':'))==2:
                     ...
                     if  se.isascii():
                        sp1=gTTS(text=spword[0],slow=False)
                        sp2=gTTS(text=spword[1],slow=False,lang='ar')
                        os.chdir(m)
                        sp1.save(f'{name}.mp3')
                        sp2.save(f'ar{name}.mp3')
                        shaw()
                        print(f'done,saved in dicnary sounds(name:{name})')
                elif len(pi)>0 and pi.lower() != 'e':
                        sp1=gTTS(text=pi,slow=False)
                        os.chdir(m)
                        sp1.save(f'{name}.mp3')
                        shaw()
      ...
      #settings
        ...
        while True:
            ...
            print('2-fix arabic in test choice')
            print('3-delete all data in favorite words')
            print('4-see more from me')
            print('5-exit')
            jk=input('enter your choice:')
            if jk=='1':
                shaw()
                with open('v.txt','r',encoding='utf-8') as vr:
                    bo=vr.read()
                    bol=bo.split(':')
                    name=bol[0]
                    nn=input(f'enter your new name(old name:{name}):')
                    vp=input('are you sure(y,n):')
                    if vp.lower()=='y':
                        with open('v.txt','w',encoding='utf-8') as vw:
                            vw.write(nn+':'+bol[1])
                    elif vp.lower() !='y':
                        pass
            elif jk=='2':
                ...
                print('1-True')
                print('2-False')
                ion=input('enter your choice:')
                if ion=='1':
                    artefi=True
                    with open('artefi.txt','w') as ar:
                        ar.write('True')
                elif ion=='2':
                    artefi=False
                    with open('artefi.txt','w') as ar:
                    ...
                    print('invalid(are you playing in settings)')              
            elif jk=='3':
                shaw()
                hj1=input('are you sure (will delete all your favourite words)(y,n):')
                if hj1.lower()=='y':
                    with open('f.csv','w',encoding='utf-8') as f:
                        pass
            elif jk=='4':
                webbrowser.open('https://share.google/C3t5VCtqZkByAY3Sr')
            elif jk=='5':
                shaw()
                break
            else:
                print('invalid')
    #eas+ter egg situation
